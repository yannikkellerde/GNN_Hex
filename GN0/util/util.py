...
def downsample_gao_outputs(q_values,target_hex_size):
    if ((len(q_values.shape)>1 and q_values.shape[1] == target_hex_size)
            or (len(q_values.shape)>1 and q_values.shape[1]==target_hex_size**2)):
        return q_values.reshape(q_values.shape[0],-1)
    if (len(q_values.shape)==1 and q_values.shape[0]==target_hex_size**2):
    ...
    if len(q_values.shape) == 1 and q_values.shape[0] == (target_hex_size+2)**2 or q_values.shape[1] == (target_hex_size+2)**2:
        ...
        assert q_values.shape[1] == target_hex_size+2, f"{q_values.shape}"
