...
def test_random_graphs(model):
    dataset = SupervisedDataset(root='./data/hexdata', device=device, pre_transform=hex_pre_transform,num_graphs=100,game_type="hex",drop=True,game_size=11)
    batch_size = 256
    ...
    with torch.no_grad():
        for i,batch in enumerate(loader):
            ...
            small_data = generate_hex_graphs(games_to_play=1,drop=True,game_size=6)[0]
...
if __name__ == "__main__":
    ...
    state_dict = torch.load("model/GraphSAGE_hex_final.pt",map_location=device)
