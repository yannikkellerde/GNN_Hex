...
def evaluate_graph(model:torch.nn.Module, graph:Graph, conversion_func:Callable, device="cpu", categorical=False):
...
def evaluate_winpattern_game_state(model:torch.nn.Module,game:Winpattern_game,device="cpu"):
    return evaluate_graph(model,game.view,convert_winpattern_game,device=device)
