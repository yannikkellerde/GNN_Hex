...
def extract_nn_update_idx_from_binary_name(current_binary_name: str) -> int:
    ...
    if match:
        try:
            ...
            logging.warn(f'Could not find cast the match when extracting epoch from binary name. '
                          f'Returning 0 (which will continue game generation/training).')
            return 0
    else:
        logging.warn(f'Could not find a match when extracting epoch from binary name. '
                      f'Returning 0 (which will continue game generation/training).')
        return 0
...
def get_current_binary_name(binary_dir: str, original_binary_name: str):
    ...
    print(os.path.abspath(f'{binary_dir}/{original_binary_name}'))
